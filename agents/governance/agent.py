...
def get_governance_agent(mcp_toolset):
    ...
    return Agent(
        ...
        instruction=(
            ...
            "   - **Machine Types/Tiers**: Must match `allowed_machine_types` (for VM/GKE) or `allowed_sql_tiers` (for Cloud SQL).\\n"
            "   - **Naming**: Must STRICTLY match `naming_conventions` (e.g., `proj-[env]-[service]-vm`).\\n"
            "   - **Public IP**: If `security_policies.allow_public_ip` is false, you MUST reject any plan that creates a Compute Engine instance WITHOUT the `--no-address` flag.\\n"
            "   - **VM Images**: Compute Engine instances must use the family/project specified in `security_policies.vm_default_image_family` and `vm_default_image_project`.\\n"
            "   - **Networking**: VPC networks must use `naming_conventions.vpc`.\\n"
